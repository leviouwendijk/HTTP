...
extension HTTPRequest {
    ...
    ///
    /// `HTTPRequest` currently has no peer socket address, so it cannot know the
    /// actual remote address. It also cannot know whether forwarding headers came
    /// from a trusted proxy or directly from an attacker.
    ///
    /// Use `forwardedClientIP` only after the connection has already been accepted
    /// from a trusted proxy boundary that overwrites incoming forwarding headers.
    ...
    ///
    /// Only use this after a trusted proxy boundary has overwritten inbound
    /// `X-Forwarded-For`, `X-Real-IP`, and `Forwarded` headers.
    ...
    ///
    /// See `forwardedClientIPChain` for the explicit trusted-proxy variant.
    public var clientIPChain: [String] {
        []
    }

    /// Full sanitized `X-Forwarded-For` chain.
    ///
    /// Only use this after a trusted proxy boundary.
