...
public struct CORSConfig: Sendable {
    public enum AllowedOrigin: Sendable {
        ...
        case matcher(Prebuilt.CORSOriginMatcher)
...
public struct CORS: Sendable {
    ...
    public func preflightResponse(
        ...
        if !config.allowedHeaders.isEmpty {
            resp.headers["Access-Control-Allow-Headers"] = config.allowedHeaders.joined(
                separator: ", "
            )
        }
    ...
    public func apply(
        ...
        if !config.exposedHeaders.isEmpty {
            resp.headers["Access-Control-Expose-Headers"] = config.exposedHeaders.joined(
                separator: ", "
            )
        }
