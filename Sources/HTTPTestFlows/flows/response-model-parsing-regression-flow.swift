...
extension HTTPFlowSuite {
    static let httpResponseModelParsingRegressionFlow = TestFlow(
        ...
        Step("model-owned response initializer matches legacy parser") {
            ...
            let legacy = try HTTPResponseParser.parse(
                raw
            )
        ...
        Step("model-owned response initializer preserves permissive policy override") {
            let raw = httpRawMessage(
                headLines: [
                    "HTTP/1.1 200 OK",
                    "Content-Length: 0",
            ...
            try Expect.equal(
                response.status,
                legacy.status,
                "response-model-parsing.policy.status"
            )

            try Expect.equal(
                response.headers,
                legacy.headers,
                "response-model-parsing.policy.headers"
            )
        }
